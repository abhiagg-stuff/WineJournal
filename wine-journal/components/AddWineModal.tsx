...
import { WineEntry, ResearchedData, WineType } from '../types';
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <input
            type="text"
            placeholder="e.g. Leroy Musigny Grand Cru"
            value={wineName}
            onChange={(e) => setWineName(e.target.value)}
            className="w-full px-4 py-2 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-red-800 focus:border-transparent"
          ...
          {previewUrl ? (
            ...
            <div className="flex flex-col items-center">
              <div className="flex space-x-4">
                <label className="cursor-pointer px-4 py-2 bg-red-800 text-white hover:bg-red-900 rounded-full flex items-center justify-center transition-colors">
                ...
                <label className="cursor-pointer px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-full flex items-center justify-center transition-colors">
                  <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex justify-end pt-6">
          ...
          <button type="submit" className="px-6 py-2 bg-red-800 text-white rounded-lg hover:bg-red-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-800">Search</button>
        </div>
      </form>
    </>
  );
...
const DetailsView: React.FC<{
  ...
  const [inCellar, setInCellar] = useState('inCellar' in initialData ? initialData.inCellar : false);
  const [wineType, setWineType] = useState<WineType>(initialData.wineType || 'unknown');
    ...
    <>
      ...
      <form onSubmit={handleSave} className="space-y-4">
        <div className="flex flex-col items-center w-full px-4 mb-8">
            <label className="text-gray-600 text-sm mb-2">Wine Type</label>
            <div className="inline-flex rounded-lg border border-gray-200 divide-x divide-gray-200">
                {(['red', 'white', 'rosé', 'sparkling', 'dessert', 'unknown'] as WineType[]).map((type) => (
                    <button
                        key={type}
                        type="button"
                        onClick={() => setWineType(type)}
                        className={`px-4 py-2 text-sm first:rounded-l-lg last:rounded-r-lg capitalize transition-colors
                            ${wineType === type
        ...
        <div className="flex flex-col items-center w-full px-4 mb-8">
            <label className="text-gray-600 text-sm mb-2">Your Personal Rating</label>
            <RatingSlider rating={userRating} onRatingChange={setUserRating} />
        </div>

        <div className="flex flex-col items-center gap-2 mb-8">
            <span className="text-gray-600 text-sm">In My Cellar</span>
            <label className="inline-flex items-center cursor-pointer">
                ...
                />
                <div className="relative w-14 h-8 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-red-800 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[4px] after:start-[4px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-red-800"></div>
            </label>
        </div>
        ...
        <div>
            <textarea 
                ...
                rows={4} 
                className="w-full px-4 py-2 border border-gray-300 rounded-2xl focus:outline-none focus:ring-2 focus:ring-red-800 focus:border-transparent" 
            />
        </div>
